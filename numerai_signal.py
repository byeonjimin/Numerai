...
def download_numerai_data(napi):
    def _load_or_download(remote, local):
...
def train_and_validate_to_diagnostics(feat_df, feats, valid_df) -> Tuple[pd.DataFrame, pd.DataFrame, List[Tuple], Optional[Dict[str, Any]]]:
...
def run_all(args):
    ...
    diag, diag_raw, models, diag_risk = train_and_validate_to_diagnostics(feat_df, feats, valid)
    ...
    if not args.no_upload:
        try:
            napi.upload_predictions(live_path, model_id=models_map[DEFAULTS["NUMERAI_MODEL_NAME"]])
